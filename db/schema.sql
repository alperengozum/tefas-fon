...
-- "Fon Tasfiye Duyurusu" bunun içinden filtrelenip aktif/pasif tespitine de katılıyor (bkz. src/lib/db.ts)
...
-- KAP'ta olmayan bildirimin kaynağı (SPK bülteni PDF'i); boşsa link KAP'taki bildirim sayfası
ALTER TABLE kap_notif ADD COLUMN IF NOT EXISTS url text;
